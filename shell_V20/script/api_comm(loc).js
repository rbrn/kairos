...
// "cmi.core.session_time"		 	CMITimespan			total time of student's sessions
...
vocipiattaforma = new Array();
	...
	vocipiattaforma[9] =  "cmi.core.session_time";
...
function LMSCommit(parameter){
	...
	try{valore = valore + escape(nomecorso) +"@";}catch(e){};
	var nameEQ = codice + "=";
	var date = new Date();
...
function LMSInitialize(parametro){
	...
	var ca = datigvp.split('@');
		for(var i=0;i < vocipiattaforma.length - 1;i++){
			var c = ca[i];
	...
	var commento = caricadati(codice + "c")
	if (commento)
		datipiattaforma[17] = commento;
	else
		datipiattaforma[17] = ""
}
...
function LMSGetValue(name){
	for (var i=0; i < vocipiattaforma.length; i++){
		if (name==vocipiattaforma[i]){
