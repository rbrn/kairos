...
function setcommento(commento,indirizzo){
	if (commento != "" && init_status != "completed" && init_status != "passed"){
		LMSSetValue("cmi.comments", commento);
		LMSCommit("");
		inviamessaggio(messaggiofeedback[11],tutorialeattivo,"messaggio");
	}
	if (commento != "" && indirizzo != ""){
	location.href = "mailto:" + indirizzo + "?Subject=Commento a "+titololo+" in modalità "+descrizionesuccessione[modofruizione]+" &Body=" + commento;
	}
}
