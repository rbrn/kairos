...
function tutoriale(blocco,provenienza){ // visualizza la pagina prescelta e gestisce il menÃÂÃÂ¹
	//alert(blocco);
	finemedium();
	...
	switch (tipotutoriale[blocco]){
		...
		default:
			document.getElementById("pulsantehistory").style.display="none";
			//alert(blocco+":"+tipotutoriale[blocco]);
	...
	if (!nodovisitato[blocco] || nodovisitato[blocco] < 1){
		...
		switch (tipolo){
			case "a":
				...
				break;
			case "b":
				if (tipotutoriale[blocco] == 2 || tipotutoriale[blocco] == 5)
					nodivisitati++;
				break;
		}
		document.getElementById("sommarioimm"+blocco).src = "shell/<? echo $path_img ?>/visto.gif";
...
function inizializza(){ // nasconde tutte le pagine tranne la home e imposta i parametri iniziali
	...
	contatore = 1; // nasconde il tutoriale e conta le pagine tutoriali significative
...
/* -- MULTIMEDIA -- */
function caricamedia(fileinput,nomefile,posizione,immagine,w,h){ // carica i file multimediali
