...
if ($aggiungi) {
	...
	for ($i=0;$i<$n;$i++) {
		...
		$query="INSERT INTO materiali_sequenza (id_risorsa,tipo_risorsa,ordine,id_corso,id_edizione,id_gruppo) VALUES ('$id_risorsa','$tipo_risorsa','$ordine','$id_corso_s','$id_edizione_s','$id_gruppo_s')";
	...
	while ($riga=$result->fetch_array()) {
		...
		if ($cancella) {
			if ($comuni=='1') {
				...
				$query2="DELETE FROM materiali_sequenza WHERE id_risorsa='$id_risorsa' AND id_corso='$id_corso_s' AND id_edizione='$id_edizione_s' AND id_gruppo='$id_gruppo_s'";
			};
		} else {
			if ($comuni=='1') {
				$query2="UPDATE materiali_sequenza SET ordine='$ordine' WHERE id_risorsa='$id_risorsa' AND id_corso=''";
			} else {
				$query2="UPDATE materiali_sequenza SET ordine='$ordine' WHERE id_risorsa='$id_risorsa' AND id_corso='$id_corso_s' AND id_edizione='$id_edizione_s' AND id_gruppo='$id_gruppo_s'";
			};
			$ordine++;
		};
...
header("Location:index.php?risorsa=materiali_sequenza_gruppo_edit&comuni=$comuni&id_corso=$id_corso_s&id_edizione=$id_edizione_s&id_gruppo=$id_gruppo_s");
?>
