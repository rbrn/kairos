...
function acquisisciparametri(){ // accetta i parametri da piattaforma e se presenti da querystring
	...
	if (stringa){
		...
		if (modocontinua){
			if (parametri[3] != 0)
				punteggio = Number(parametri[3]);
			for(var i=4;i < parametri.length;i++){
				if (parametri[i] != null)
					nodovisitato[nometutoriale[i-3]] = Number(parametri[i])
			}
		}
	}
	...
	switch (direzione){
		case 1: // avanti
			if (tipotutoriale[tutorialeattivo] != 2)
				...
				switch (tipolo){
					case "a":
						if (nodovisitato[tutorialeattivo] == 3 || ((nodovisitato[tutorialeattivo] == 2 || nodovisitato[tutorialeattivo] == 4) && bloccoavanzamento == 0))
							tutoriale(successivo[tutorialeattivo]);
						else
...
function tutoriale(pagina,ritorno){ // visualizza la pagina prescelta e gestisce il menù
	...
	if (tutorialealternativo[blocco] && (multimedia == 0 || plugin_fl ==0))
	...
	switch (tipotutoriale[blocco]){// tipo tutoriale
		...
		case 2: // test
			switch (nodovisitato[blocco]){// tipo tutoriale
				case 2:
					inviamessaggio(messaggiofeedback[24],blocco,"messaggio");
					break;
				case 3:
					inviamessaggio(messaggiofeedback[25],blocco,"messaggio");
					break;
				case 4:
					inviamessaggio(messaggiofeedback[24],blocco,"messaggio");
					break;
		...
		case 4: // riepilogo
			...
			if (classepunteggio > 4){
				document.getElementById("feedback_"+blocco).src = "riepilogo_01.gif";
				...
				document.getElementById("feedback_"+blocco).src = "riepilogo_02.gif";
				...
				document.getElementById("feedback_"+blocco).src = "riepilogo_03.gif";
				...
				document.getElementById("feedback_"+blocco).src = "riepilogo_04.gif";
				document.getElementById("feedback_"+blocco).alt = messaggiofeedback[4];
				inviamessaggio(messaggiofeedback[9],blocco);
			}
			else{
				document.getElementById("feedback_"+blocco).src = "riepilogo_05.gif";
	...
	if (tutorialeprincipale[blocco] != null){// indice
		document.getElementById("sommarioimm_"+tutorialeprincipale[blocco]).src = "../immagini/visto.gif";
		...
		document.getElementById("sommarioimm_"+blocco).src = "../immagini/visto.gif";
...
function comandi(pagina,tipotest,rispostaesatta,tutorialetest,messaggio,animazione){ //inserisce i comandi nascosti per gestire audio, video e animazioni
	...
	switch (tipotutoriale[pagina]){
		case 2:
			if (tipotest != 7){ // diverso da cruciverba
			...
			if (tipolo != "c"){ // diverso da solo test
				...
				if (animazione){
					...
					document.write("<a href='#' accesskey='s' tabindex='160' onClick="+stringa+" onKeyPress="+stringa+">");
					document.write("<img class='pulsanteanimazione' onMouseOver='roll(this,\"animazione\")' onMouseOut='noroll(this,\"animazione\")' src='../immagini/pulsante_animazione.gif' alt='Rivedi'></a>");
				}
			}
			break;
		case 5:
...
function inizializza(){ // nasconde tutte le pagine tranne la home e imposta i parametri iniziali
	...
	while (nometutoriale[contatore]){
		...
		if (nodovisitato[nometutoriale[contatore]] > 0 && tutorialeprincipale[nometutoriale[contatore]] == null){
			document.getElementById("sommarioimm_"+nometutoriale[contatore]).src = "../immagini/visto.gif";
...
function traccia(){ // memorizza i dati
	...
	var dati = dimensionicarattere + "," + dimensionipagina + "," + multimedia+ "," + punteggio;
	for(var i=1;i < nometutoriale.length;i++){
			if (nodovisitato[nometutoriale[i]] > 0){
				if (nodovisitato[nometutoriale[i]] == 4)
...
function pulsantiambiente(){
	...
	if (dimensionipagina==2)
		document.write("<a href='#' onClick='diminuiscizoom();return false;' onKeypress='diminuiscizoom();return false;'><img class='pulsantezoom' onMouseOver=\"roll(this,'zoom')\" onMouseOut=\"noroll(this,'zoom')\"  src='../immagini/pulsante_zoom.gif' alt='Rimpicciolisci la finestra'><\/a>");
	else
		document.write("<a href='#' onClick='aumentazoom();return false;' onKeypress='aumentazoom();return false;'><img class='pulsantezoom' onMouseOver=\"roll(this,'zoom')\" onMouseOut=\"noroll(this,'zoom')\" src='../immagini/pulsante_zoom.gif' alt='Ingrandisci la finestra'><\/a>");
	document.write("<\/span>");
	document.write("<a href='#' onClick='incrementa();return false;' onKeypress='incrementa();return false;'><img class='pulsanteingrandisci' onMouseOver=\"roll(this,'ingrandisci')\" onMouseOut=\"noroll(this,'ingrandisci')\" src='../immagini/pulsante_ingrandisci.gif' alt='Ingrandisci i caratteri'><\/a>");
	document.write("<a href='#' onClick='decrementa();return false;' onKeypress='decrementa();return false;'><img class='pulsanterimpicciolisci' onMouseOver=\"roll(this,'rimpicciolisci')\" onMouseOut=\"noroll(this,'rimpicciolisci')\"  src='../immagini/pulsante_rimpicciolisci.gif' alt='Rimpicciolisci i caratteri'><\/a>");
	document.write("<a id='pulsantesuonoon' href='#' onClick='attivamedium();return false;' onKeypress='attivamedium();return false;'><img class='pulsantesuono' onMouseOver=\"roll(this,'suonoon')\" onMouseOut=\"noroll(this,'suonoon')\" src='../immagini/pulsante_suonoon.gif' alt=\"Attiva audio, video e animazioni\"><\/a>");
	document.write("<a id='pulsantesuonoff' href='#' onClick='disattivamedium();return false;' onKeypress='disattivamedium();return false;'><img class='pulsantesuono' onMouseOver=\"roll(this,'suonooff')\" onMouseOut=\"noroll(this,'suonooff')\" src='../immagini/pulsante_suonooff.gif' alt=\"Disattiva audio, video e animazioni\"><\/a>");
}
...
function pulsantinavigazione(){
	...
	document.write("<a id='pulsantehistory' href='#' accesskey='u' tabindex='900' onClick='indietrohistory();return false;' onKeypress='indietrohistory();return false;'><img id='history' onMouseOver=\"roll(this,'history')\" onMouseOut=\"noroll(this,'history')\" src='../immagini/pulsante_history.gif' alt=\"Torna all'ultima pagina di test o al riepilogo\"><\/a>");
	document.write("<span id='pulsantecontinua'><a href='#' accesskey='q' tabindex='115' onClick=\"tutoriale('"+init_bookmark+"');return false;\" onKeyPress=\"tutoriale('"+init_bookmark+"');return false;\"><img id='continua' onMouseOver=\"roll(this,'continua')\" onMouseOut=\"noroll(this,'continua')\" src='../immagini/pulsante_continua.gif' alt='Continua il corso al punto in cui è stato interrotto'></a><\/span>");
}
