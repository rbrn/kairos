...
{
	if (isHTMLMode){return;}
	win_open('materiali_apri_immagine.php','Immagini',500,500);
}
...
{
if (isHTMLMode){return;}
  win_open('materiali_apri_suono.php','Suoni',500,500);
}

function InserisciFlash()
{
if (isHTMLMode){return;}
  win_open('materiali_apri_flash.php','Flash',500,500);
}

function InserisciVideo()
{
if (isHTMLMode){return;}
  win_open('materiali_apri_video.php','Flash',500,500);
}

function CreaLink()
{
if (isHTMLMode){return;}
  win_open('crea_link.php','Link',600,300);
}
...
function InsertLink(url,risorsa,tipo0,tipo1, x, y,target0,target1) {
   ...
   if (rng!=null) {
		...
		if (risorsa!='') url='http://ec2-54-229-184-60.eu-west-1.compute.amazonaws.com/kairos/kairos/materiali_browse.php?risorsa='+risorsa;

		if (tipo1) {

		link='<a href=\"javascript:nota(\''+url+'\',\''+x+'\',\''+y+'\')\">'+etichetta+'</a>';
...
function InsertSound(url) {
   ...
   var etichetta=rng.text;
	...
	link='<a href=\"javascript:suona(\''+nome+'\')\">'+etichetta+'</a>';
