...
function setup_posizione($id_padre) {
	...
	$pos=1;
	$query="SELECT id_risorsa,tipo FROM risorse_materiali WHERE risorsa_padre='$id_padre' and (tipo='0' or tipo='1' or tipo='2' or tipo='3') ORDER BY posizione,id_risorsa";
...
$query = "SELECT id_risorsa,posizione FROM risorse_materiali WHERE risorsa_padre='$risorsa_padre' and (tipo='0' or tipo='1' or tipo='2' or tipo='3') ORDER BY posizione,id_risorsa";
$result=$mysqli->query($query);
...
while ($riga=$result->fetch_array() and $continua) {
	...
	$query_succ = "SELECT id_risorsa,posizione FROM risorse_materiali WHERE risorsa_padre='$risorsa_padre' AND posizione>$pos_att ORDER BY posizione,id_risorsa";
